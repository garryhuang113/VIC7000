...
def SetGasParam(**kwargs):
	...
	try:
		...
		return {'error_message':'Cannot find T_ID.', 'is_success':'N'}
	...
	Gas_param[T_ID] = json.dumps(Gas_param_temp)
...
def RecognizeImage(**args):
	...
	E_ID = TOOL.OCR(0, 'epmm_ID', x1, y1, w1, h1, img, {'segmentation':7, 'preprocess_resize':7, 'preprocess_resize_method': 0, 'preprocess_threshold_method':1, 'preprocess_threshold_algorithm': 1, 'preprocess_threshold_value': 135})
	U_ID = TOOL.OCR(0, 'epmm_ID', x2, y2, w2, h2, img, {'segmentation':7, 'preprocess_resize':7, 'preprocess_resize_method': 0, 'preprocess_threshold_method':1, 'preprocess_threshold_algorithm': 1, 'preprocess_threshold_value': 135})	
	...
	if find == 1:
		...
		Gas_dict[T_ID] = json.dumps(Gas_dict_temp)
		Gas_return[T_ID] = json.dumps(Gas_return_temp)
		...
		return {'result':'Data for T_ID = ' + T_ID +', E_ID = ' + E_ID + ', and U_ID = ' + U_ID + ' is ready.', 'is_success':'Y'}
	
	else:
		return {'error_message':'Cannot find data matching E_ID = ' + E_ID + ', and U_ID = ' + U_ID + ' in stored T_ID.', 'is_success':'N'}
...
def GetGasData(**kwargs):
	...
	if (T_ID in Gas_return.keys() and T_ID in Gas_dict.keys()) and T_ID in Gas_param.keys():

		return_dict['data'] = Gas_return[T_ID]
		return_dict['is_success'] = 'Y'
		
		lock.acquire()
		
		Gas_dict.pop(T_ID, None)
		Gas_return.pop(T_ID, None)
		Gas_param.pop(T_ID, None)
		...
		return {'error_message':'Cannot find data matching T_ID = ' + T_ID + '.', 'is_success':'N'}
